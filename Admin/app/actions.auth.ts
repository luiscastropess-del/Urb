...
export async function getUserSession() {
  try {
    ...
    if (!token) {
      console.warn("getUserSession: No token found");
      return null;
    }

    const verified = await jwtVerify(token, SECRET_KEY).catch(e => {
    ...
    console.log("getUserSession: Token verified for user ID:", payload.id);
    ...
    if (!user) {
      console.warn("getUserSession: User not found in database for id:", payload.id);
    }
