  ...
  if (data.images) {
    try {
      ...
      if (Array.isArray(parsedImages)) {
        const uploaded = await Promise.all(parsedImages.map(img => processImageUpload(img)));
        data.images = JSON.stringify(uploaded);
      }
    } catch (e) {
...
export async function getAdminDashboardStats() {
  const [placesCount, citiesCount, usersCount, guidesCount] = await Promise.all([
    db.place.count(),
    db.city.count(),
    db.user.count(),
